...
/// in the application that may convert between a [SpaceRelativePath] and a
/// [RepoRelativePath] — everywhere else, the two types keep the mix-up from
/// compiling.
@freezed
abstract class Space with _$Space {
  ...
  RepoRelativePath? get rootWithinRepository {
    ...
    final String trimmed = within.startsWith('/')
        ? within.substring(1)
        : within;
    return trimmed.isEmpty ? null : RepoRelativePath(trimmed);
  }
  ...
  RepoRelativePath toRepoRelative(SpaceRelativePath path) {
    final RepoRelativePath? prefix = rootWithinRepository;
    return prefix == null
        ? RepoRelativePath(path.value)
        : RepoRelativePath('${prefix.value}/${path.value}');
  }
  ...
  SpaceRelativePath? toSpaceRelative(RepoRelativePath path) {
    final RepoRelativePath? prefix = rootWithinRepository;
    if (prefix == null) {
      return SpaceRelativePath(path.value);
    }
    final String inside = '${prefix.value}/';
    return path.value.startsWith(inside)
        ? SpaceRelativePath(path.value.substring(inside.length))
        : null;
  }
  ...
  /// platform recognises.
  String absolutePathOf(SpaceRelativePath path) {
