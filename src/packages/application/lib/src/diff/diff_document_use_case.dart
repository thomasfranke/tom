...
/// The comparison of the document on screen against a revision of itself.
///
/// The after side arrives already parsed, because the preview has just split
/// it to draw it and a second parse per keystroke is not worth paying.
final class DiffDocumentUseCase with UseCase {
  ...
  /// failure: a new file and an unborn `HEAD` are both normal.
  Future<Result<DocumentDiffValueObject, AppFailure>> diff({
    ...
    // Spelled out because the body answers in two vocabularies and inference
    // lands on `Object?` between them.
