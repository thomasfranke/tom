...
final class ReadDocument with UseCase {
  ...
  Future<Result<ParsedDocument>> call(Space space, SpaceRelativePath path) =>
      guard(() async {
        final Result<Document> read = await documentsFor(space).read(path);
        return switch (read) {
          Success<Document>(value: final Document document) =>
            await blocks.read(document),
          Failure<Document>(failure: final AppFailure failure) =>
            Failure<ParsedDocument>(failure),
        };
      });
}
