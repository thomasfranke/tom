...
2. **The architecture and the patterns are the review.** They come from `docs/technical/architecture.md`, `docs/technical/conventions/` and `docs/technical/runtime/`, from the decisions in `docs/technical/decisions/`, and from the dartdoc of the code that already implements them — and nothing in the build catches them.
...
- **`architecture.md` and everything in `conventions/` are normative.** Every other technical doc describes current practice. Deviating from a normative section requires a new file in `docs/technical/decisions/` in the same change — never a silent exception.
...
- Single-field identifier/path wrapper (`BranchNameValueObject`, `CommitShaValueObject`, `RepoRelativePathValueObject`) → `extension type`, *recommended* — a Freezed single-field class is a note, not a finding. A wrapper that grows a second field is the trigger to revisit.
- Use cases, repositories, infra implementations, notifiers, widgets are not data — leave them alone.
- **Freezed compares collections element-wise but does not copy them.** A Freezed field holding a `List` is handed over, not owned: the producer passes `List.unmodifiable` and the field's dartdoc states it (`GitStatusValueObject.entries`, `GitMergeConflict.conflictedFiles`). A new collection field without both is a finding. Making it structural needs an immutable-collection package — license check plus an ADR — and is the maintainer's call, not a PR's.
- A `@Assert` is evaluated in the generated part and cannot see a static: its helpers are private top-level functions (`SpaceEntity` shows the shape). A constructor that stops being `const` for the sake of an invariant is the right trade and says so.

### Paths and vocabulary (Decision 15, rule 12)

**A space is a folder, not a repository.** `SpaceEntity` carries `root` and `repositoryRoot` separately; git runs against the repository, navigation, search and the watcher stay inside the folder.

- `RepoRelativePathValueObject`, `SpaceRelativePathValueObject` and absolute `String` paths are distinct types on purpose — path confusion is the most probable bug in this application. A new signature taking a `String` where one of the two relative types belongs is a finding.
- `SpaceEntity` is the only converter: `toRepoRelative`, `toSpaceRelative` (null for what the space does not contain — an ordinary answer, handled, never `!`-ed), `relativize` (from the absolute paths the filesystem reports), `absolutePathOf`. A conversion written anywhere else is a finding. `SpaceRelativePathValueObject.resolve` is the one place a document's own `..` is folded — a link or an image is read from the document's folder, and a second resolver in a widget is a finding.
- The documentation's words are the code's words: Space, Document, Block, DiffBlock, SpaceEntry — carried into the code with the role word [Decision 23](../../../docs/technical/decisions/023-domain-types-say-entity-or-value-object.md) mandates, `SpaceEntity`, `DocumentEntity`, `BlockValueObject`, `SpaceEntryValueObject`. No `FileModel`, no `DocDto`, and no bare `Space` either: `tom rules naming` fails a domain type that says neither. A concept renamed in code is renamed in `docs/` in the same PR.

### Tests (`conventions/testing.md`, normative)
...
3. **Clean Architecture and DDD, as `architecture.md`, `conventions/` and Decision 15 apply them.**
...
**A role word in a name is different from a pattern word, and the test is what the reader needs in order to use the thing.** A role is part of the contract: `SpaceRepository`, `OpenSpaceUseCase`, `HomeState`, `FileTreePanel`, `GitFailure`, `MarkdownParser` — drop the word and the name stops saying what it is for. A pattern word describes plumbing the code already makes obvious, and those stay out: `Model`, `Manager`, `Helper`. Three suffixes that look like pattern words are role words here, by decision: `Dto` and `Dao` when the shape is real ([Decision 21](../../../docs/technical/decisions/021-dtos-and-daos-when-they-are-real.md)), and `Entity`/`ValueObject` on every domain type ([Decision 23](../../../docs/technical/decisions/023-domain-types-say-entity-or-value-object.md)) — the distinction is a behavioural contract the reader cannot see otherwise, so `SpaceEntity`, `DocumentEntity` and `BlockValueObject` are the names, and a domain type without either is a finding the tool already raises. Use cases carry `UseCase` in the class *and* the file (`open_space_use_case.dart`) and are invoked through a named method, which is the shape of the Flutter team's own architecture sample; `conventions/naming.md` holds the rule.

**DDD, selectively (Decision 15).** Invariants are validated once, at construction — a value object that can be built invalid is a finding, as is a check repeated downstream of a type that already guarantees it. No primitive obsession where a type exists: a `String` path, a `String` branch, a `String` sha in a new signature. Entities compare by identity (`DocumentEntity` by path), value objects by value; a hand-written equality that confuses the two is a bug, not a style. The ubiquitous language is enforced by naming, in both directions: an infrastructure word in the domain (`process`, `file handle`, `row`) is as much a finding as a `Dto` suffix. A domain service is stateless and exists only for logic that belongs to no single entity — `BlockDiffer`, and possibly nothing else for a long while.
...
**`tom_domain`** — entities, value objects, the sealed area failures, repository contracts, `BlockDiffer`. Knows no framework, no git, no disk, no package type. Domain contracts (`GitRepository`, `DocumentRepository`, `SpaceRepository`) speak in entities and typed paths — never text, never a `GitClientFailure`. Identity by path (`DocumentEntity`), not by content. The DDD menu is chosen (Decision 15): value objects for identifiers and paths, ubiquitous language, repositories, one domain service — and no aggregates, no domain events, no factories or specifications, no separate persistence model. `BlockValueObject` is an output of Spike B: **do not design `BlockDiffer` before the spike reports**, and a diff that does is a finding by the roadmap. A contract keeps its role word and takes neither suffix — `BlockReaderPort` is a port, `SpaceRepository` a repository — which is also what the naming rule exempts, by file suffix.

**`tom_infra`** — capability contracts and their implementations, organised **by capability, not by technology**: `git_client/` holds the contract, its sealed failure, and one subfolder per implementation named after how it is done (`dart_io/`, later `libgit2/`). Depends on `tom_core` and on `tom_data` for the DTOs that cross its contracts ([Decision 24](../../../docs/technical/decisions/024-a-capability-is-a-folder.md)) — never on `tom_domain`, so it cannot name a domain failure; do not try. On a contract:
...
- **Panels are registered, never hardcoded** (Decision 12): a `TomModule` contributes `PanelDescriptor`s and provider overrides, `runTom(modules: [...])` collects them, and the built-in panels go through an internal `CoreModuleImpl` on exactly the same path. A panel wired straight into the shell widget is a finding even when it is the only panel. Modules add; they never change or degrade what the app does, and none may need the network to start. The app never imports a module.
