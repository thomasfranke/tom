...
**A role word in a name is different from a pattern word, and the test is what the reader needs in order to use the thing.** A role is part of the contract: `SpaceRepository`, `OpenSpaceUseCase`, `HomeState`, `FileTreePanel`, `GitFailure`, `MarkdownParser` — drop the word and the name stops saying what it is for. A pattern word describes plumbing the code already makes obvious, and those stay out: `Dto`, `Dao`, `Entity`, `Model`, `Manager`, `Helper`. `Space`, `Document` and `Block` are entities and are named exactly that, because the documentation's words are the code's words. Use cases carry `UseCase` in the class *and* the file (`open_space_use_case.dart`) and are invoked through a named method, which is the shape of the Flutter team's own architecture sample; `layers.md` holds the rule.
