...
The seam has one known hazard, and it is a question for Spike B: reference links and footnotes are defined at document scope (`[foo]` in one place, `[foo]: url` at the bottom), so a block rendered in isolation loses them unless the document's reference map travels with it. See [08-domain-model.md](../domain/model.md).
