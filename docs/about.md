...
> **TOM — Team-Oriented Markdown** *(binary: `tom`)*. A Git client specialized
> in documentation. Your markdown files, your Git repository, and a UI that
> makes that workflow pleasant for an entire team — no mandatory cloud, no
> lock-in, no subscription.

What this project is, who it is for, and what it refuses to become — the
context every audience needs before the documentation forks between product
and engineering.

**This file stays short and stable.** Feature rules belong in
...
> This section is the pitch-level summary. The detailed, evolving behavior rules for each feature live in [product/](product/README.md), one folder per feature — that is what implementation should be checked against.
...
> **Not a non-goal: mobile.** iOS and Android are a planned direction, post-1.0 — desktop still comes first, and nothing about the MVP scope changes. See the [roadmap](tasks/roadmap.md#phases) and [Decision 8](technical/decisions/008-monorepo-with-pure-dart-core.md), which already reserves the shape (`tom_infra_mobile` + `apps/tom_mobile` over the same pure-Dart core).

---

*See also: [product/](product/README.md) · [technical/](technical/README.md) · [tasks/roadmap.md](tasks/roadmap.md)*
