...
- A feature flag guards **registration**, not rendering ([feature flags](../patterns/feature-flags.md)): a disabled panel is never registered, so it is unreachable rather than merely invisible.
- Changing `TomModule` so that an existing module stops compiling is a breaking change ([versioning](../versioning.md)).
