...
The principle is not uniform: external services get full isolation, third-party widgets get a wrapper we own, and structural dependencies (Flutter, Riverpod, Freezed) are a declared exception with a restricted scope rather than an abstraction. The three tiers, their boundaries and why the exception exists are in [layers.md](../architecture/layers.md#external-dependencies).
