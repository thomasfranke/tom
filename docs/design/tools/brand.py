"""The identity: the icon, the wordmark and the lockup, as data.
    ...
    python3 docs/design/tools/brand.py                 # -> ../brand/*.svg, the masters and the sheet
    python3 docs/design/tools/brand.py --rasters DIR   # + PNG 16–1024, .icns, .ico
...
# ── The sheet: ../brand/brand.svg, embedded by brand/README.md ───────────
def sheet():
    cells = []
    # light half
...
def write_masters():
    ...
    for name, svg in masters().items():
        with open(os.path.join(BRAND_DIR, name), "w", encoding="utf-8") as f:
            f.write(svg + "\n")
    with open(os.path.join(BRAND_DIR, "brand.svg"), "w", encoding="utf-8") as f:
        f.write(sheet() + "\n")
...
def main(argv):
    ...
    write_masters()
    print(f"{len(masters())} masters -> {os.path.relpath(BRAND_DIR)}  ·  sheet -> {os.path.relpath(os.path.join(BRAND_DIR, 'brand.svg'))}")
    if args.rasters:
