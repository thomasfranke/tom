...
| `flutter_markdown_plus` | BSD-3 | Renders the inline content **inside** a block — **in use**, `^1.0.12`. One `MarkdownBody` per block, never one for the document: block-level layout and the container around each block are ours, which is what the rendered diff needs ([runtime/documents.md](../runtime/documents.md)) |
