...
And one test, [`src/test/architecture_test.dart`](../../../src/test/integrity/architecture_test.dart), for the failure neither the compiler nor a lint can see: a dependency **added to a pubspec**, after which the illegal import is entirely legal. It reads every pubspec and asserts the table above, including that exactly one package knows Flutter exists. Changing the graph means changing that test first — deliberate friction, because the graph is a decision.
