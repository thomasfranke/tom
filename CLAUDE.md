...
- `docs/architecture/` — the system design, one folder per package mirroring `src/packages/`, plus `overview/` for what crosses every layer
...
- **Code templates live beside the layer they govern**: `architecture/core/error-handling.md`, `architecture/presentation/dependency-injection.md`, `architecture/presentation/extension-modules.md`, `architecture/overview/feature-flags.md`. All new code follows them.
...
4. **No `dartz`** — Result with native sealed classes, exhaustive switch, early return (Decision 5); canonical template in `docs/architecture/core/error-handling.md` (inline try/catch mandatory in every use case).
5. **No navigation package** — plain `Navigator` for dialogs only (Decision 6).
6. **External dependencies isolated behind contracts** in infrastructure (Decision 7); Riverpod ONLY in presentation + bootstrap/di — no `Ref` in any other layer; constructor injection (templates in `docs/architecture/presentation/dependency-injection.md`).
...
10. **Extensible shell** — panels are ALWAYS registered through `TomModule`/`PanelDescriptor`, never hardcoded in the shell, including the built-in ones (Decision 12; template in `docs/architecture/presentation/extension-modules.md`).
...
See `docs/architecture/overview/repository-structure.md` (source of truth). The Dart workspace lives under `src/`; run everything through `make` from the repository root.
...
- **Incomplete work integrates behind a build-time feature flag**, never on a long-lived branch (`docs/architecture/overview/feature-flags.md`). Disabled = unreachable, and every flag has a removal target.
